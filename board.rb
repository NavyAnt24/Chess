...
class Board
  ...
  def move(from_pos, to_pos)
    ...
    if valid_move?(from_pos, to_pos) && !check?(piece.color)
      ...
      raise "This puts #{piece.color}'s king in check!"
    end
  end
  ...
  def check?(current_color)
    ...
    opp_pieces = get_opponent_pieces(opp_color)
    my_king_position = get_king_position(current_color)
    valid_move = false
    opp_pieces.each do |opp_piece|
      begin
        valid_move = valid_move?(opp_piece.position, my_king_position)
        return valid_move if valid_move == true
      rescue Exception
      end
    end
    return valid_move
  ...
  def checkmate?(current_color)
    my_pieces = get_opponent_pieces(current_color) #get all of my pieces
    possible_moves = []
    my_pieces.each do |piece|
      piece.possible_moves.each do |possible_move|
        next if !valid_move?(piece.position, possible_move)
  ...
  def get_opponent_pieces(opponent_color)
    opp_pieces = []
    iterate_board do |piece|
